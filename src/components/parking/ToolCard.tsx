      ...
      <div className="flex-1 min-w-0 flex flex-col gap-1.5">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-sm font-semibold text-neutral-100 truncate">{tool.name}</span>
            {tool.url && (
              <a href={tool.url} target="_blank" rel="noopener noreferrer"
                onClick={(e) => e.stopPropagation()}
                className="text-neutral-500 hover:text-accent opacity-0 group-hover:opacity-100 transition-all shrink-0 mt-0.5">
          ...
          <div className="flex items-center gap-1 shrink-0">
            {isOwner && (
              <button
                onClick={handleEdit}
                className="opacity-0 group-hover:opacity-100 p-1 rounded-lg hover:bg-surface-400 text-neutral-600 hover:text-accent transition-all"
              >
                <Pencil size={13} />
              </button>
            )}
            {isOwner && (
              <button
                ...
                className="opacity-0 group-hover:opacity-100 p-1 rounded-lg hover:bg-red-500/10 text-neutral-600 hover:text-red-400 transition-all"
              >
                <Trash2 size={13} />
              </button>
            )}
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {(tool.categories || ['other']).map((cat) => {
            const catColor = CATEGORY_COLORS[cat]
            return (
              <span key={cat} className={cn('text-[10px] px-2 py-0.5 rounded-full font-medium', catColor.bg, catColor.text)}>
                {CATEGORY_LABELS[cat]}
              </span>
            )
          })}
